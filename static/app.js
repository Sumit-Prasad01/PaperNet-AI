...
async function predictCoraNode(nodeId) {
  ...
  try {
    ...
    // Pair probabilities with class mapping (dynamic from server or project default)
    const classMapping = window.serverClassMapping || {
      0: "Rule_Learning",
      1: "Neural_Networks",
...
async function initModelInfo() {
  ...
  try {
    ...
    if (data.class_mapping) {
      window.serverClassMapping = data.class_mapping;
    }
    const modelName = data["Model Name"] || data.model_name || "PaperNet-AI Advanced GATv2";
    specFile.textContent = `${modelName} (advanced_cora_gatv2.onnx)`;
